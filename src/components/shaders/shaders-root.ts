...
export const LAVA_LAMP_DARK_SOURCE = Skia.RuntimeEffect.Make(`
uniform float time;
uniform vec2 size;
...
vec4 main(vec2 fragCoord) {
    ...
    // Blue accent (#3563E9) for blobs
    vec3 blobColor = vec3(0.21, 0.39, 0.91);
...
export const LAVA_LAMP_LIGHT_SOURCE = Skia.RuntimeEffect.Make(`
uniform float time;
uniform vec2 size;
...
vec4 main(vec2 fragCoord) {
    ...
    // Off-white background for light mode
    vec3 bgColor = vec3(0.97);

    // Softer blue accent for light mode
    vec3 blobColor = vec3(0.21, 0.39, 0.91);

    // Mix background with blob color - softer intensity
    vec3 finalColor = mix(bgColor, blobColor, blob * 0.4);
