/**
 * useEraseFlow -- hook for the Erase Tattoo flow.
 * Pick an image with a tattoo, erase it via AI.
 */

import { buildErasePrompt } from "@/lib/flowPrompts";
import {
  cacheBase64Image,
...
export function useEraseFlow() {
  ...
  async function erase() {
    ...
    customEvent("generation_started", { type: "erase" });

    const base64 = await getCachedImageAsBase64(sourceUri);
    mutation.mutate({
      prompt: buildErasePrompt(),
